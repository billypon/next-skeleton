...
Object.assign(Axios.defaults, {
  baseURL: publicRuntimeConfig.PUBLIC_API,
})
...
function logServerErrorData(data: string | object = '-----', indent = 2): void {
  const indentStr = Array.from({ length: indent }, () => ' ').join('')
  if (typeof data === 'string') {
    console.error(`${ indentStr }${ data }`)
  } else {
    console.error(JSON.stringify(data, null, indent).replace(/^/g, indentStr))
  }
}

function logServerError({ config, response }: AxiosError): void {
  const { url, method, data, params, headers } = config
  console.error(`[${ (new Date).toLocaleString() }] ${ method } ${ url }`)
  logServerErrorData(params)
  if (data) {
    logServerErrorData()
    logServerErrorData(data)
  }
  logServerErrorData()
  logServerErrorData(headers)
  if (response) {
    logServerErrorData()
    logServerErrorData(response.data)
  }
}
